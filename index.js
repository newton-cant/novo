	...
	const checkRegisteredUser = (sender) => {
		...
		//WAKTU
			...
			var ase = new Date();
                ...
                case 8: jamss = "Bomdia ☀️"; break;
                case 9: jamss = "Bomdia ☀️"; break;
                case 10: jamss = "Bomdia ☀️"; break;
                ...
                case 16: jamss = "Boatarde ☀️"; break;
...
case 'menu':
  case 'help':  
  if (isBanned) return  
 if (!isSupraReg) return sendButMessage(from, lang.noregis(pushname2), `Clique no botão para verificar`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif}); 
sendButImage(from, lang.menu(prefix, salam, pushname2), '© ' + ownername, thumbnail, [{buttonId: `${prefix}owner`, buttonText: {displayText: '🪀 𝐎𝐖𝐍𝐄𝐑'}, type: 1},{buttonId: `${prefix}info`, buttonText:{displayText: '📒 𝐈𝐍𝐅𝐎'}, type: 1}], {quoted: supra2})
 ...
 sendButMessage(from, `*LhannaBOT*`, `*_Aguarde um pouco_* 💤`, [{buttonId: `${prefix}ok`,buttonText: {displayText: `OK`,},type: 1,}], {quoted: mek});
   ...
   shaa = `Waifu️`
  sendButImage(from, shaa, 'Clique no botão abaixo para obter uma imagem aleatória de waifu ', sads, [{buttonId: `${prefix}waifu`, buttonText: {displayText: 'Outra imagem'}, type: 1}], {quoted: supra2})			
  break	
...
const buttons = [{buttonId: '${prefix}antidelete on', buttonText: {displayText: 'ON✔️'}, type: 1},{buttonId: '${prefix}antidelete off', buttonText: {displayText: 'OFF✖️'}, type: 1}]
const buttonMessage = {
    contentText: `${anu}`,
...
await supra.sendMessage(from, buttonMessage, MessageType.buttonsMessage, {quoted: ftrol})
 ...
 sendButMessage(from, `*LhannaBOT*`, `*_Aguarde um pouco_* 💤`, [{buttonId: `${prefix}ok`,buttonText: {displayText: `OK`,},type: 1,}], {quoted: mek});
   ...
   shaa = `Loli️`
  sendButImage(from, shaa, 'Clique no botão abaixo para obter uma imagem aleatória de loli ', sads, [{buttonId: `${prefix}loli`, buttonText: {displayText: 'Outra imagem'}, type: 1}], {quoted: supra2})			
  break	
...
Comentários:️ ${data.data.comment}`
  sendButVideo(from, shaa, 'Clique no botão abaixo para obter um video aleatório de Gore ', sads, [{buttonId: `${prefix}Gore ${hay}`, buttonText: {displayText: '⏩Proximo'}, type: 1}], {quoted: supra2})			
  break	
 b37802bfba
//f216290a2c23  
  case 'hentaivideo':
...
Tranf:️ ${data.result.share_count}
...
shaa = `*Dados obtidos️ com sucesso!!*
Titulo: ${supragost.title}
Duração: ${respf.result.duration}
Info: ${supragost.info}`
  sendButImage(from, shaa, '*Fiz para Elaine*\n_Se deseja baixar clique no botão abaixo_ ', sads, [{buttonId: `${prefix}baixarxvideos ${supragost.link}`, buttonText: {displayText: 'Baixar'}, type: 1}], {quoted: supra2})			
  break	
...
┃ *➣ Fabricação de disp* : ${device_manufacturer}
...
let scen = await supra.sendMessage(from, fs.readFileSync(`./jadibot@${sender}.jpg`), MessageType.image, {quoted : mek,caption: 'Digitalize este QR para se tornar um bot temporário!\n1. Clique nos três pontos no canto superior direito\n2. Toque em WhatsApp Web\n3. Digitalize este QR\n\nQR Expira em 20 segundos'})    
setTimeout(() => {
...
if (isBanned) return
  try{
        ...
        if (args.length < 2) return supra.sendMessage(from, 'Cadê o texto irmão?', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                if (err) return reply('falhou irmão :(')
        ...
        gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                fs.unlinkSync(ranm)
                buff = fs.readFileSync(rano)
                if (err) return reply(`Api não funciona`)
                supra.sendMessage(from, buff, audio, {quoted:mek,ptt:true})
                fs.unlinkSync(rano)
            })
        })
...
*_Lhanna está baixando_* 🎶🎶`
 supra.sendMessage(from, buffer, image, { quoted: supra2, thumbnail: fs.readFileSync('./image/supraf.jpg'), caption: fdsz })
supra.sendMessage(from, Iagu, audio, {mimetype: 'audio/mp4', ptt:true, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": `${anu.title}️`, 'jpegThumbnail': await getBuffer(anu.thumb)}}}})
   break		
...
➞ *Visualização* : ${anu.result[0].views}
...
sendButMessage(from, `*LhannaBOT*`, `*_Aguarde um pouco_* 💤`, [{buttonId: `${prefix}ok`,buttonText: {displayText: `OK`,},type: 1,}], {quoted: mek});
 var hay= args.join(' ')
   ...
   wfd = await getBuffer(anu.result.url)
   supra.sendMessage(from, wfd, audio, {mimetype: 'audio/mp4', ptt:true, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": `${anu.result.title}️`, 'jpegThumbnail': await getBuffer(anu.result.thumb)}}}})
   break
